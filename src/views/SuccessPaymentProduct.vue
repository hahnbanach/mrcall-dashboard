<template>
  <div class="min-h-full flex flex-column justify-content-between">
    <div class="payment-content-section main-page-content-section flex-shrink-0 text-center">
      <p class="title md:text-4xl text-3xl mb-3">{{ $t('views.successpaymentproduct.failedPayment') }}</p>

      <Button @click="router.push('/businesses')"
              class="px-6 py-3 md:w-auto w-full"
              icon="pi pi-fw pi-box" :label="$t('views.successpaymentproduct.gotoAssistant')" />
    </div>
    <footer id="footer">
    ...
    if(! this.store.state.user) {
      onAuthStateChanged(auth, (user) => {
        if (user && user.emailVerified && !user.isAnonymous) {
          ...
          this.$gtag.event("product_payment_succeeded", {
